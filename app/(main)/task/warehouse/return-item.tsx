...
const ReturnItemScreen = () => {
  ...
  const [returnList, setReturnList] = useState<BatchInput[]>([]);
  ...
  const handleAddItem = () => {
    ...
    setReturnList((prev) => [...prev, currentItem]);
    setCurrentItem({ barcode: "", qty_done: 0, notes: "" });
  };

  const handleSubmit = () => {
    console.log("Submit Barang Retur:", returnList);
  ...
  return (
    <View style={styles.page}>
      ...
      <ThemedHeader title="Catat Barang Retur" />
      <View style={styles.container}>
        <ThemedKeyboardAvoiding withFlex={false}>
          ...
          <ThemedInput
            label="Jumlah Retur"
            placeholder="Masukkan jumlah barang retur"
          ...
          <ThemedTextarea
            label="Alasan Retur"
            placeholder="Tuliskan alasan atau catatan retur (opsional)"
            multiline
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={styles.batchItem}>
              <ThemedText type="Bold">Retur #{index + 1}</ThemedText>
          ...
          ListEmptyComponent={
            <View style={{ marginVertical: scale(80) }}>
              ...
              >
                Belum ada barang retur dicatat
              </ThemedText>
            </View>
          }
        />
